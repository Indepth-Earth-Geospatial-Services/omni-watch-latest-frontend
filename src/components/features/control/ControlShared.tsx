...
export const CmdButton = ({
  ...
  const styles: Record<string, string> = {
    default:
      'bg-secondary/50 border-border/50 text-muted-foreground hover:bg-secondary/60 hover:border-border hover:text-foreground',
    danger:
...
export const SectionHeader = ({ title }: { title: string }) => (
  <p className='text-[9px] font-black tracking-[0.18em] text-muted-foreground uppercase mb-1.5'>
    {title}
  </p>
);
...
export const FormInput = ({
  ...
  <div className='flex flex-col gap-0.5'>
    ...
    <div className='flex items-center gap-1'>
      ...
      />
      {unit && (
        <span className='text-[9px] text-muted-foreground flex-shrink-0 font-mono'>{unit}</span>
      )}
    </div>
  </div>
);
