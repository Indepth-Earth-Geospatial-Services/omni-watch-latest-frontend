...
const AccordionHeader = ({
  ...
  <button
    ...
    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg border transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
      expanded ? colorClass : 'bg-card border-border/50 hover:border-zinc-600'
    }`}
  >
    <div className='flex items-center gap-2'>
      <Icon size={12} className={expanded ? '' : 'text-muted-foreground'} />
      <div className='text-left'>
        <p className='text-[10px] font-black uppercase tracking-widest leading-none'>{title}</p>
        <p className='text-[8px] text-muted-foreground mt-0.5'>{subtitle}</p>
      </div>
    </div>
    {expanded ? (
      ...
      <div className='flex flex-col gap-3'>
        ...
        <div
          className={`flex items-center justify-between rounded-lg border px-3 py-2.5 transition-colors ${
            flightAuth ? 'bg-blue-500/10 border-blue-500/40' : 'bg-card border-border/50'
          }`}
        >
          <div className='flex-1 min-w-0 mr-3'>
            ...
            {joystickInvalidState && (
              <p className='text-[9px] font-logs text-red-400/70 mt-0.5'>
                Lost:{' '}
        ...
        <div>
          ...
          />
          {expanded === 'flyto' && (
            <div className='mt-1.5 bg-background rounded-lg border border-border/40 p-3 flex flex-col gap-2.5'>
          ...
          {expanded === 'stop' && (
            <div className='mt-1.5 bg-background rounded-lg border border-border/40 p-3 flex flex-col gap-2.5'>
              ...
              <div className='flex items-center gap-2'>
                ...
                <span
                  className={`text-[9px] font-logs uppercase tracking-wide ${
                    drcStatus === 'active'
                      ? 'text-emerald-400'
                      : drcStatus === 'connecting'
                        ? 'text-amber-400'
                ...
                {drcStatus === 'active' ? (
                  <button
                    ...
                    className='ml-auto text-[8px] font-bold uppercase tracking-wider text-muted-foreground hover:text-foreground border border-zinc-600/40 hover:border-zinc-400/60 rounded px-1.5 py-0.5 transition-colors'
                  >
                    Exit DRC
                  </button>
        ...
        <div className='rounded-lg bg-secondary/50 border border-border/40 px-3 py-2.5'>
          ...
          <div className='flex flex-col gap-1'>
            {Object.entries(JOYSTICK_INVALID_REASONS).map(([code, label]) => (
              <div key={code} className='flex items-center gap-2'>
                <span className='text-[9px] font-logs text-zinc-700 w-3 flex-shrink-0'>{code}</span>
