...
import { Navigation2, Check, Loader2, AlertTriangle, Route } from 'lucide-react';
            ...
            {isLoading ? (
              <div className='space-y-1'>
                {Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className='px-2.5 py-2 rounded-lg bg-neutral-900/40 space-y-1.5'
                  >
              ...
              {isLoadingRoute ? (
                ...
                <>
                  <span className='text-[9px] text-cyan-400 font-mono'>
                    {activeRoute.length} pts · {Math.min(...activeRoute.map((p) => p.alt).filter(Boolean)).toFixed(0)}–
                    {Math.max(...activeRoute.map((p) => p.alt)).toFixed(0)} m
                  </span>
                  <button
                    onClick={(e) => { e.stopPropagation(); onFitRoute(); setOpen(false); }}
                    className='text-[9px] font-bold text-cyan-400 hover:text-cyan-300 transition-colors ml-2'
                  >
                    FIT MAP
                  </button>
                </>
