...
const ThreatMap = dynamic(
  ...
  {
    loading: () => (
      <div className='h-48 animate-pulse bg-neutral-800 rounded-lg border border-border/50' />
    ),
    ssr: false,
  }
);
...
export function AlertDetailModal({
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center'>
      ...
      {/* Modal */}
      <div className='relative bg-background border border-border/50 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4'>
        {/* Header */}
        <div className='flex items-center justify-between p-4 border-b border-border/50'>
          <div className='flex items-center gap-2'>
            <span className='font-semibold text-foreground capitalize'>{d.type}</span>
            <span
              className={cn(
            ...
            {d.status === 'dismissed' && (
              <span className='text-xs px-2 py-0.5 rounded bg-zinc-600/30 text-muted-foreground font-bold uppercase'>
                Dismissed
              </span>
            )}
          </div>
        ...
        <div className='p-4 space-y-4'>
          {/* Image */}
          {d.imageUrl && (
            <div className='relative rounded-lg overflow-hidden border border-border/50 bg-secondary h-[300px]'>
              <Image
                src={d.imageUrl}
          ...
          <div className='bg-secondary/50 rounded-lg p-3 border border-border/50'>
            <h4 className='text-xs font-medium text-muted-foreground mb-1'>Bounding Box</h4>
          ...
          <div className='grid grid-cols-2 gap-3'>
            ...
            <div className='bg-secondary/50 rounded-lg p-3 border border-border/50'>
              ...
              <p className='text-sm font-mono text-foreground truncate'>{d.streamId}</p>
            </div>
            ...
            <div className='bg-secondary/50 rounded-lg p-3 border border-border/50'>
              ...
              <p className='text-sm text-foreground'>{formatTimeAgo(d.detectedAt)}</p>
            </div>
            ...
            <div className='bg-secondary/50 rounded-lg p-3 border border-border/50'>
              ...
              <p className='text-sm font-mono text-foreground'>#{d.trackId}</p>
            </div>
          </div>
          ...
          {d.isVerified && d.reasoning && (
            <div className='bg-secondary/50 rounded-lg p-3 border border-border/50'>
              <h4 className='text-xs font-medium text-theme-accent mb-2'>LLM Reasoning</h4>
          ...
          {d.droneLatitude != null && d.droneLongitude != null && (
            <a
              href={geospatialHref}
              className='flex items-center justify-center gap-2 w-full py-2.5 rounded-lg border border-border text-sm text-foreground hover:bg-secondary/50 transition-colors'
            >
